...
PAD_TOKEN = 0
POINT_MIRROR={0:0,1:3,3:1,2:2,4:6,6:4,5:5,7:9,9:7,8:8}
...
class MultiTaskLSTM(nn.Module):
    ...
    def forward(self, X, lengths):
        ...
        score = self.attn(o).squeeze(-1)   # (B, T)
        score = score.masked_fill(~mask, -1e9)
        w = torch.softmax(score, dim=1).unsqueeze(-1)
        attn_hidden = (o * w).sum(dim=1)
        return self.act_head(o), self.pt_head(o), self.rly_head(attn_hidden).squeeze(1)
...
def normalize_perspective(df,left_hand_code):
    df = df.copy().sort_values(["rally_uid", "strikeNumber"])
    df["receiver_hand"] = df.groupby("rally_uid")["handId"].shift(-1)
    left_mask = df["receiver_hand"] == left_hand_code
    df.loc[left_mask, "pointId"] = df.loc[left_mask, "pointId"].map(POINT_MIRROR)
    return df
def main(args):
    ...
    act_w = torch.tensor(1.0/act_counts, dtype=torch.float32); act_w = act_w ** 0.5;act_w = (act_w * (n_act/act_w.sum()))
    pt_w  = torch.tensor(1.0/pt_counts,  dtype=torch.float32); pt_w = pt_w ** 0.5;pt_w  = (pt_w  * (n_pt /pt_w.sum()))
    ...
    for ep in range(1, args.epochs+1):
        ...
        if final > best_final:
                ...
                print(f"  ✓ Best model saved (Final={final:.4f})")
        else:
            no_improve += 1
            ...
            if no_improve >= patience:
                ...
                break
    model.load_state_dict(torch.load(f"best.pt"))
    model.eval()
    print(f"Loaded best model (Final={best_final:.4f})")
    for i, (feat, emb) in enumerate(zip(FEATURES, model.embs)):
        norm = emb.weight.norm(dim=1).mean().item()
        print(f"{feat}: avg embedding norm = {norm:.4f}")
    # inference
    ...
    single_rally = [rid for rid, g in test.groupby("rally_uid") if len(g) == 1]
    single_preds = pred_df[pred_df["rally_uid"].isin(single_rally)]
    print("單拍rally的pointId預測分布:")
    print(single_preds["pointId"].value_counts().sort_index())
    ...
    print(pd.Series(pred_df["pointId"]).value_counts())
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="best.csv")
